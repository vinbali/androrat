...
public class Protocol {
	...
	public static byte[] dataHeaderGenerator(int totalLenght, int localLength, boolean moreF,
		short idPaquet, int channel) {
		final byte[] byteTotalLength = ByteBuffer.allocate(4).putInt(totalLenght).array();
		final byte[] byteLocalLength = ByteBuffer.allocate(4).putInt(localLength).array();
		final byte[] byteMoreF = new byte[1];
		if (moreF)
			byteMoreF[0] = 1;
		else
			byteMoreF[0] = 0;
		final byte[] bytePointeurData = ByteBuffer.allocate(2).putShort(idPaquet).array();
		final byte[] byteChannel = ByteBuffer.allocate(4).putInt(channel).array();

		final byte[] header = new byte[HEADER_LENGTH_DATA];

		System.arraycopy(byteTotalLength, 0, header, 0, byteTotalLength.length);
		System
			.arraycopy(byteLocalLength, 0, header, byteTotalLength.length, byteLocalLength.length);
		System.arraycopy(byteMoreF, 0, header, byteTotalLength.length + byteLocalLength.length,
			byteMoreF.length);
		System.arraycopy(bytePointeurData, 0, header, byteTotalLength.length
			+ byteLocalLength.length + byteMoreF.length, bytePointeurData.length);
		System.arraycopy(byteChannel, 0, header, byteTotalLength.length + byteLocalLength.length
			+ byteMoreF.length + bytePointeurData.length, byteChannel.length);
