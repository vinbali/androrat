...
public class wavIO {
	...
	public static int byteArrayToInt(byte[] b) {
		final int start = 0;
		final int low = b[start] & 0xff;
		final int high = b[start + 1] & 0xff;
		return (high << 8 | low);
	}
	...
	public String getSummary() {
		...
		final String summary = "<html>Format: " + this.myFormat + newline + "Channels: "
			+ this.myChannels + newline + "SampleRate: " + this.mySampleRate + newline
			+ "ByteRate: " + this.myByteRate + newline + "BlockAlign: " + this.myBlockAlign
			+ newline + "BitsPerSample: " + this.myBitsPerSample + newline + "DataSize: "
			+ this.myDataSize + "</html>";
		return summary;
	}
	...
	public boolean read() {
		...
		this.myData = null;
		final byte[] tmpLong = new byte[4];
		final byte[] tmpInt = new byte[2];

		try {
			inFile = new DataInputStream(new FileInputStream(this.myPath));

			// System.out.println("Reading wav file...\n"); // for debugging
			// only

			final String chunkID = "" + (char) inFile.readByte() + (char) inFile.readByte()
				+ (char) inFile.readByte() + (char) inFile.readByte();

			inFile.read(tmpLong); // read the ChunkSize
			this.myChunkSize = byteArrayToLong(tmpLong);

			final String format = "" + (char) inFile.readByte() + (char) inFile.readByte()
				+ (char) inFile.readByte() + (char) inFile.readByte();
			...
			final String subChunk1ID = "" + (char) inFile.readByte() + (char) inFile.readByte()
				+ (char) inFile.readByte() + (char) inFile.readByte();

			inFile.read(tmpLong); // read the SubChunk1Size
			this.mySubChunk1Size = byteArrayToLong(tmpLong);

			inFile.read(tmpInt); // read the audio format. This should be 1 for
									// PCM
			this.myFormat = byteArrayToInt(tmpInt);

			inFile.read(tmpInt); // read the # of channels (1 or 2)
			this.myChannels = byteArrayToInt(tmpInt);

			inFile.read(tmpLong); // read the samplerate
			this.mySampleRate = byteArrayToLong(tmpLong);

			inFile.read(tmpLong); // read the byterate
			this.myByteRate = byteArrayToLong(tmpLong);

			inFile.read(tmpInt); // read the blockalign
			this.myBlockAlign = byteArrayToInt(tmpInt);

			inFile.read(tmpInt); // read the bitspersample
			this.myBitsPerSample = byteArrayToInt(tmpInt);
			...
			final String dataChunkID = "" + (char) inFile.readByte() + (char) inFile.readByte()
				+ (char) inFile.readByte() + (char) inFile.readByte();

			inFile.read(tmpLong); // read the size of the data
			this.myDataSize = byteArrayToLong(tmpLong);

			// read the data chunk
			this.myData = new byte[(int) this.myDataSize];
			inFile.read(this.myData);
	...
	public void readRaw() {
		try {
			final File f = new File(this.myPath);
			final DataInputStream inFile = new DataInputStream(new FileInputStream(f));
			this.myDataSize = (int) f.length();
			this.myData = new byte[(int) this.myDataSize];
			inFile.read(this.myData);
			inFile.close();
		} catch (final Exception e) {
	...
	public boolean save() {
		try {
			final DataOutputStream outFile = new DataOutputStream(new FileOutputStream(this.myPath));

			// write the wav file per the wav file format
			outFile.writeBytes("RIFF"); // 00 - RIFF
			outFile.write(intToByteArray((int) this.myChunkSize), 0, 4); // 04 -
																			// how
																			// big
																			// is
																			// the
			...
			outFile.write(intToByteArray((int) this.mySubChunk1Size), 0, 4); // 16
																				// -
																				// size
																				// of
																				// this
																				// chunk
			outFile.write(shortToByteArray((short) this.myFormat), 0, 2); // 20
																			// -
																			// what
																			// is
																			// the
																			...
																			// Code
																			// Modulation
			outFile.write(shortToByteArray((short) this.myChannels), 0, 2); // 22
																			// -
																			// mono
																			// or
																			// stereo?
																			...
																			// or
																			// ???)
			outFile.write(intToByteArray((int) this.mySampleRate), 0, 4); // 24
																			// -
																			// samples
																			// per
																			// second
																			...
																			// per
																			// second)
			outFile.write(intToByteArray((int) this.myByteRate), 0, 4); // 28 -
																		// bytes
																		// per
																		// second
			outFile.write(shortToByteArray((short) this.myBlockAlign), 0, 2); // 32
																				// -
																				// #
																				// of
																				// bytes
																				...
																				// all
																				// channels
			outFile.write(shortToByteArray((short) this.myBitsPerSample), 0, 2); // 34
																					// -
																					// how
																					// many
																					// bits
			...
			outFile.write(intToByteArray((int) this.myDataSize), 0, 4); // 40 -
																		// how
																		// big
																		// is
																		// this
																		// data
																		// chunk
			outFile.write(this.myData); // 44 - the actual data itself - just a
	...
	public boolean save2() {
		try {
			...
			// write the wav file per the wav file format
			outFile.writeBytes("RIFF"); // 00 - RIFF
			outFile.writeInt(Integer.reverseBytes((int) this.myChunkSize)); // 04
																			// -
																			// how
																			// big
																			// is
			...
			outFile.writeBytes("WAVE"); // 08 - WAVE
			outFile.writeBytes("fmt "); // 12 - fmt
			outFile.writeInt(Integer.reverseBytes((int) this.mySubChunk1Size)); // 16
																				// -
																				// size
																				// of
																				// this
																				// chunk
			outFile.writeShort(Short.reverseBytes((short) this.myFormat)); // 20
																			// -
																			// what
																			// is
																			// the
																			...
																			// Code
																			// Modulation
			outFile.writeShort(Short.reverseBytes((short) this.myChannels)); // 22
																				// -
																				// mono
																				// or
																				// stereo?
																				...
																				// or
																				// ???)
			outFile.writeInt(Integer.reverseBytes((int) this.mySampleRate)); // 24
																				// -
																				// samples
																				// per
																				// second
																				...
																				// per
																				// second)
			outFile.writeInt(Integer.reverseBytes((int) this.myByteRate)); // 28
																			// -
																			// bytes
																			// per
																			// second
			outFile.writeShort(Short.reverseBytes((short) this.myBlockAlign)); // 32
																				// -
																				// #
																				// of
																				// bytes
																				...
																				// all
																				// channels
			outFile.writeShort(Short.reverseBytes((short) this.myBitsPerSample)); // 34
																					// -
																					// how
																					// many
																					// bits
																					...
																					// 24
			outFile.writeBytes("data"); // 36 - data
			outFile.writeInt(Integer.reverseBytes((int) this.myDataSize)); // 40
																			// -
																			// how
																			// big
																			// is
	...
	public void setHeaders() {
		...
		this.mySubChunk1Size = 16; // taille fmt (on a pas d'option)
		this.myFormat = 1;
		this.myChannels = 1;
		this.mySampleRate = 11025; // prise dans la config android
		this.myByteRate = 20050; // 11025 * 2 car 11025 nb samples 1 sec et 1
									// sample 16 donc 2 bytes
		this.myBlockAlign = 2; // 1 sample 2 byte
